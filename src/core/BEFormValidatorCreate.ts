...
export class BEFormValidatorCreate implements BEFormValidatorCreateImpl
{
    ...
    private inputsTreatment(needleInputs = this.__initialInputs, isCustom: boolean = false)
    {
        ...
        for (let i = 0; i < needleInputs.length; i++) {
            if (
                needleInputs[i].element instanceof NodeList
                || needleInputs[i].element instanceof Array
            ) {
                const commonOptions: AValidateInput = {...needleInputs[i]};
                // @ts-ignore
                const elements = Array.from(needleInputs[i].element);
                needleInputs.splice(i, 1);
                ...
                inputsResult.push(needleInputs[i]);
            }
        }
    ...
    ): void
    {
        ...
        // @ts-ignore
        const result = this.inputsTreatment(inputS, true);
